      ...
      {({ open }) => (
        <>
          <div className="relative w-1/4">
            <Listbox.Button className="relative w-full cursor-pointer bg-[color:rgba(var(--background),1)] py-5 pl-3 pr-2 text-left  focus:outline-none">
              <span className="flex items-center">
                <img src={selectedCountry.flag} alt="" className="h-8 w-8 flex-shrink-0 sm:h-5 sm:w-5 md:w-5 md:h-5 lg:w-6 lg:h-6" />
                <span className="ml-3 block truncate font-light text-xl">{selectedCountry.dialCode}</span>
              </span>
            </Listbox.Button>
            ...
            <Transition show={open} as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
              <Listbox.Options
                id="countryscollbar"
                className="countryscollbar absolute z-10 mt-1 max-h-64 w-[280%] overflow-auto rounded-md bg-[color:rgba(var(--background),1)] py-1 -ml-1 text-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none"
              >
                {Data.map((country) => (
                  <Listbox.Option
                    key={country.isoCode}
                        ...
                        <div className="flex items-center">
                          ...
                          <span
                            className={classNames(
                              ...
                              'ml-3 block truncate font-light max-[333]:text-md text-xl',
                            )}
                          >
                            {country.isoCode}
                          </span>
