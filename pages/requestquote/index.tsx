...
const RequestQuote2 = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    setIsLoading(true);
    try {
      let response = await fetch(`https://ntem3igx14.execute-api.ap-northeast-1.amazonaws.com/dev/quote/create`, {
        method: 'POST',
  ...
  const getProductDetails = async (hsQuery: any) => {
    try {
      let response = await fetch(`https://ntem3igx14.execute-api.ap-northeast-1.amazonaws.com/dev/search/hscodes?q=${hsQuery}`);
      let data = await response.json();
      console.log(data);
  ...
  const getLocation1 = async (cityQuery1: any) => {
    ...
    try {
      let response = await fetch(`https://ntem3igx14.execute-api.ap-northeast-1.amazonaws.com/dev/searchcity/cities?q=${cityQuery1}`);
      let data = await response.json();
      console.log(data);
      setCities1(data.hits.hits);
    } catch (e) {
      console.log(e);
    }
  };
  ...
  const getLocation2 = async (cityQuery2: any) => {
    try {
      let response = await fetch(`https://ntem3igx14.execute-api.ap-northeast-1.amazonaws.com/dev/searchcity/cities?q=${cityQuery2}`);
      let data = await response.json();
      setCities2(data.hits.hits);
    } catch (e) {
      console.log(e);
    }
  };
  ...
  return (
    <div className="relative my-12 mx-[3%] lg-mx-[15%] md:mx-[10%] sm:mx-[5%] py-6 px-8">
      <Head>
      ...
      <form onSubmit={(e) => handleSubmit(e)}>
        <div className="mb-6 lg:w-8/12 sm:w-full">
          ...
          <div className="mb-6">
            <label htmlFor="first_name" className="block mb-3 text-2xl font-medium">
              Product <span className="text-[red]">*</span>
            </label>
            <div className="flex flex-col relative">
              <div className="border relative rounded-xl border-gray-300 focus:outline-[#4F46E5] hover:border-[#4F46E5] flex justify-between items-center gap-3 px-3">
                <svg
                ...
                <input
                  ...
                  id="product_details"
                  autoComplete="off"
                  className="text-xl bg-[color:var(--inputBackground)]  block w-full p-5 focus:outline-none "
              ...
              {hsCodesLoading ? (
                <div className="border m-auto absolute w-full mt-[4.5rem] h-40 z-30 bg-[color:rgba(var(--background),1)] flex place-items-center rounded-xl">
              ...
              {productDetails && productDetails.length > 0 ? (
                <div className="border absolute w-full mt-[4.5rem] max-h-72 z-30 bg-[color:rgba(var(--background),1)] overflow-y-scroll overflow-x-hidden rounded-xl">
                  {productDetails.map((product) => {
                    return (
                      <div
                        key={product._id}
                        onClick={() => {
                          handleClickProductName(product._source.name, product._source.description, product._source.hscode);
                          setFormData({
                            ...formData,
                        ...
                        className="text-xl cursor-pointer font-medium px-6 py-4 whitespace-nowrap hover:bg-[#4F46E5] hover:text-white"
                      >
                        <h2>
                          {product._source.name} &nbsp;&nbsp;&nbsp; {product._source.description} &nbsp; {product._source.hscode}
                        </h2>
                      </div>
                    );
                  })}
              ...
              <button
                ...
                className={
                  seaSelected
                    ...
                    : 'group px-6 pt-2.5 pb-2 bg-[color:rgba(var(--background),1)] font-medium text-xl leading-normal uppercase rounded-xl shadow-md hover:bg-[#4F46E5] hover:text-white hover:shadow-lg focus:bg-[#4F46E5] focus:text-white focus:shadow-lg focus:outline-none focus:ring-0 active:shadow-lg transition duration-150 ease-in-out flex align-center'
                }
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="grid gap-6 mb-6 md:grid-cols-2">
            <div className="flex flex-col relative">
              <div className="relative">
                <label htmlFor="from_location" className="block mb-3 text-2xl font-medium">
                ...
                <input
                  ...
                  className="bg-[color:var(--inputBackground)] rounded-lg border border-gray-300 text-2xl focus:outline-[#4F46E5] hover:border-[#4F46E5] block w-full p-5 mb-2"
              ...
              {cities1 ? (
                <div
                  id="locfrom"
                  className="border absolute shadow-lg w-full mt-[72px] max-h-48 z-30 bg-[color:rgba(var(--background),1)] overflow-y-scroll overflow-x-hidden rounded-md countryscollbar"
                >
                  {cities1.map((city) => {
                    return (
                      <div
                        onClick={() => {
                          setCityQuery1('');
                          handleClickCity1(city._source.name, city._source.country);
                          setCities1(null!);
                        }}
                      ...
                      >
                        <h2>
                          {city._source.name} : {city._source.country}
                        </h2>
                      </div>
                    );
                  })}
            ...
            <div className="flex flex-col relative">
              <div className="relative">
                <label htmlFor="last_name" className="block mb-3 text-2xl font-medium">
              ...
              {cities2 ? (
                <div
                  id="locto"
                  className="border absolute w-full mt-[72px] max-h-48 z-30 bg-[color:rgba(var(--background),1)] overflow-y-scroll overflow-x-hidden rounded-md countryscollbar"
                >
                  {cities2.map((city) => {
                    return (
                      <div
                        onClick={() => {
                          setCityQuery2('');
                          handleClickCity2(city._source.name, city._source.country);
                          setCities2(null!);
                        }}
                        className="text-lg cursor-pointer font-medium px-6 py-4 whitespace-nowrap hover:bg-[#4F46E5] hover:text-white"
                        key={city._id}
          ...
          <div className="grid gap-6 mb-6 md:grid-cols-2">
            <div>
              <label htmlFor="date" className="block mb-3 text-2xl font-medium">
          ...
          <div className="mb-10">
            <label htmlFor="additional_information" className="block mb-3 text-2xl font-medium">
          ...
          <div className="grid gap-6 mb-6 md:grid-cols-2">
            <div>
              <label htmlFor="First Name" className="block mb-3 text-2xl font-medium">
            ...
            <div>
              <label htmlFor="Last Name" className="block mb-3 text-2xl font-medium">
          ...
          <div className="grid gap-6 mb-8 md:grid-cols-2">
            <div>
              <label htmlFor="phone" className="block mb-3 text-2xl font-medium">
                Phone <span className="text-[red]">*</span>
              </label>
              <div className="flex justify-between gap:2 items-center bg-[color:var(--inputBackground)] rounded-lg border border-gray-300 text-sm focus:outline-[#4F46E5] hover:border-[#4F46E5] w-full">
                ...
                <input
                  ...
                  className="bg-[color:var(--inputBackground)] text-2xl block w-8/12 p-5 focus:outline-none "
            ...
            <div>
              <label htmlFor="Email" className="block mb-3 text-2xl font-medium">
          ...
          <div className="flex items-center mb-8">
            <div className="flex items-center h-5">
              <input
                ...
                type="checkbox"
                value=""
                className="w-7 h-7 bg-[color:var(--inputBackground)] rounded-sm  border border-gray-300 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800"
                required
              />
            </div>
            <label htmlFor="remember" className="ml-2 text-2xl font-medium dark:text-gray-400 flex">
          ...
          {isLoading ? (
            ...
            <button
              type="submit"
              className="bg-[#4F46E5] text-white hover:bg-[#4F46E5] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium text-2xl w-full sm:w-auto px-8 py-4 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
            >
              Submit
            </button>
          )}
